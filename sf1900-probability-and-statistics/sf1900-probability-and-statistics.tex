...
\begin{enumerate}[label=\bfseries (\alph*)]
    ...
    \item \textbf{Normal (or Gaussian)}. $f_X(x) = \frac{1}{\sigma \sqrt{2 \pi}} e^{-(x-m)^2 / 2 \sigma^2} \quad (-\infty < x < \infty)$, wher $m$ and $\sigma$ are given quantities $(\sigma > 0)$. Code name $X \sim N(m, \sigma^2)$
